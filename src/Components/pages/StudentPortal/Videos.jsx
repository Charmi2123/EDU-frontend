...
import { ThumbsUp, MessageSquare, Share2, Bookmark, Flag, Send, X, Reply, Info, Settings, RotateCcw, RotateCw, Volume, Gauge, Monitor, Volume2, VolumeX, Maximize, Play, RefreshCw, RefreshCcw, Settings2, Search, ArrowRight, User, ChevronLeft } from "lucide-react";
...
function Videos({ isDark, toggleTheme, sidebardata }) {
    ...
    const handleRelatedVideoClick = (video) => {
        const id = video?.id;
        if (!id) return;
        navigate(`/StudentPortal/Videos`, { state: { video } });
    };
    ...
    const fetchSharePeers = async () => {
        const token = getToken();
        if (!token) {
            setShareError("No authentication token found");
            return;
        }
        ...
        try {
            ...
            setSharePeers(Array.isArray(peers) ? peers : []);
        } catch (err) {
            console.error("Failed to fetch chat peers", err);
            setShareError(err?.response?.data?.message || "Failed to load classmates");
        } finally {
    ...
    const openShareModal = async () => {
        ...
        setShareError(null);
        if (!sharePeers || sharePeers.length === 0) {
            await fetchSharePeers();
        }
    };
    ...
    // Share current video to selected peers via API
    const handleShareConfirm = async () => {
        try {
            ...
            const selectedEnrollments = Object.entries(selectedRecipients)
                .filter(([, v]) => Boolean(v))
            ...
            // API expects one peer per call; send sequentially
            for (const enrollment of selectedEnrollments) {
                await axios.post(
                    `${BACKEND_API_URL}/school-portal/videos/${videoId}/share`,
                    { peer_enrollment: String(enrollment), message: "" },
                    {
                        headers: {
                            accept: "application/json",
                ...
                <main className="mt-4 md:mt-6 flex-1 flex flex-col min-h-0 px-4 md:px-8">
                    {loading ? (
                        ...
                        <div className="relative flex-1 min-h-0">
                                    ...
                                    <div className={`border ${panelBg} rounded-xl px-3 py-2 sm:px-4 sm:py-3 md:px-5 md:py-2`}>
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            ...
                                            <button
                                                ...
                                                className={`group flex items-center gap-0 sm:gap-2 h-9 px-2 sm:px-3 rounded-md cursor-pointer transition ${showShareModal
                                            ...
                                            >
                                                <Share2 className={`w-4 h-4 ${showShareModal ? '' : (isDark ? 'text-white' : 'text-black')}`} />
                                                <span className={`hidden sm:inline font-inter font-semibold text-base leading-none tracking-normal capitalize ${showShareModal ? '' : (isDark ? 'text-white' : 'text-black')}`}>Share</span>
                                            </button>
                                            <button
                                                    ...
                                                    <div className="flex items-start gap-3">
                                                        <div className="flex-1">
                                                            <textarea
                                                                ...
                                                                rows={3}
                                                                className={`w-full resize-none rounded-md border px-3 py-2 outline-none no-scrollbar ${isDark ? 'bg-zinc-800 border-zinc-800 placeholder-zinc-500 focus:ring-2 focus:ring-[#2563EB] focus:border-transparent' : 'bg-white border-zinc-300 placeholder-zinc-400 focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent'}`}
                                                            />
                                                        </div>
                                                    </div>
                            ...
                            {/* Share Drawer (Right Side) - match PlayedVideo UI */}
                            <div
                                className={`fixed inset-0 z-50 transition-opacity duration-300 ${showShareModal ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
                            >
                                {/* Backdrop */}
                                <div
                                ...
                                {/* Drawer Content */}
                                <div
                                    className={`absolute top-0 right-0 h-full w-full max-w-[400px] transition-transform duration-500 ease-out transform ${showShareModal ? 'translate-x-0' : 'translate-x-full'} ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'}`}
                                >
                                    <div className="flex flex-col h-full relative">
                                        ...
                                        <div className="flex-1 overflow-y-auto mt-4 px-2 no-scrollbar">
                                            {shareLoading ? (
                                                <div className="py-6 text-center text-sm opacity-70">Loading students...</div>
                                            ) : shareError ? (
                                                <div className="py-6 text-center text-sm text-red-500">{shareError}</div>
                                            ) : (
                                                (sharePeers || [])
                                                    .filter(p => {
                                                        const q = shareSearch.toLowerCase();
                                                        return (
                                                            (p.name || p.student_name || '').toLowerCase().includes(q) ||
                                                            String(p.enrollment_number || '').toLowerCase().includes(q)
                                                        );
                                                    })
                                                    .map((p) => {
                                                        const displayName = p.name || p.student_name || p.enrollment_number;
                                                        const enrollment = String(p.enrollment_number || '').trim();
                                                        const selected = Boolean(selectedRecipients[enrollment]);
                                                        return (
                                                            <div
                                                                key={enrollment || displayName}
                                                                onClick={() => setSelectedRecipients(prev => ({ ...prev, [enrollment]: !prev[enrollment] }))}
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div className="flex flex-col text-left">
                                                                        ...
                                                                        {/* {enrollment && <span className="text-xs opacity-70">{enrollment}</span>} */}
                                                                    </div>
                                                                </div>
                                                                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${selected
                                                                    ? (isDark ? 'bg-white border-white' : 'bg-zinc-800 border-zinc-800')
                                                                    : 'border-zinc-300'
                                                                    }`}>
                                                                    {selected && (
                                        ...
                                        <div className="absolute bottom-10 right-6">
                                            <button
                                                ...
                                                className={`w-14 h-14 rounded-full flex items-center justify-center transition-all transform active:scale-95 duration-200 ${Object.values(selectedRecipients).filter(Boolean).length > 0 && !isSharing
                                                ...
                                                disabled={Object.values(selectedRecipients).filter(Boolean).length === 0 || isSharing}
                                            >
                                                <ArrowRight size={28} className={isDark && Object.values(selectedRecipients).filter(Boolean).length > 0 ? "text-black" : "text-white"} />
                                            </button>
                                        </div>
                                    </div>
                                </div>
