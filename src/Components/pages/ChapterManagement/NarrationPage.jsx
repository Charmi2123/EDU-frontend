...
import { Pencil, Mic, Paperclip, Camera, Send, PencilOff, RotateCcw, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, ArrowLeft, RotateCw } from "lucide-react";
...
function NarrationPage({ theme = 'dark', isDark: isDarkProp, toggleTheme, sidebardata, backto = "/chapter" }) {
    ...
    // Per-slide drafts so user can edit multiple slides before saving
    const [draftNarrations, setDraftNarrations] = useState({});
    // Per-slide undo/redo history
    const [histories, setHistories] = useState({}); // { [slideIndex]: string[] }
    const [historyIndex, setHistoryIndex] = useState({}); // { [slideIndex]: number }
    ...
    const handlePrevSlide = () => {
        if (currentSlideIndex > 0) {
            ...
            const original = slides[nextIndex]?.narration || "";
            const draft = draftNarrations?.[nextIndex];
            setNarration(draft ?? original);
            setOriginalNarration(original);
        }
    };
    ...
    // Seed history when entering edit mode or switching slides while in edit mode
    useEffect(() => {
        if (!isManualEdit || slides.length === 0) return;
        const si = currentSlideIndex;
        setHistories((prev) => {
            if (prev[si]?.length) return prev;
            return { ...prev, [si]: [narration || ""] };
        });
        setHistoryIndex((prev) => {
            if (typeof prev[si] === 'number') return prev;
            return { ...prev, [si]: 0 };
        });
    }, [isManualEdit, currentSlideIndex]);

    const pushHistory = (si, value) => {
        setHistories((prev) => {
            const curList = prev[si] || [narration || ""];
            const idx = (historyIndex[si] ?? (curList.length - 1));
            // If we are not at the end, truncate redo branch
            const base = curList.slice(0, idx + 1);
            if (base[base.length - 1] === value) return prev; // no duplicate
            const nextList = [...base, value].slice(-100); // cap history size
            return { ...prev, [si]: nextList };
        });
        setHistoryIndex((prev) => {
            const curList = histories[si] || [narration || ""];
            const idx = (prev[si] ?? (curList.length - 1));
            const nextIdx = Math.min(idx + 1, (histories[si]?.length ?? 0));
            return { ...prev, [si]: nextIdx };
        });
    };

    const handleUndo = () => {
        const si = currentSlideIndex;
        const idx = historyIndex[si] ?? 0;
        if (idx <= 0) return;
        const list = histories[si] || [];
        const newIdx = idx - 1;
        const value = list[newIdx] ?? "";
        setHistoryIndex((prev) => ({ ...prev, [si]: newIdx }));
        setNarration(value);
        setDraftNarrations((prev) => ({ ...prev, [si]: value }));
    };
    ...
    return (
        <div className={`relative flex ${isDark ? 'bg-zinc-950 text-gray-100' : 'bg-[#F5F5F9] text-zinc-900'} min-h-screen overflow-y-hidden transition-colors duration-300`}>
            ...
            {/* Content wrapper to allow blurring during loading */}
            <div className={`${isLoadingSlides ? 'blur-md' : ''} transition-all duration-200 flex w-full`}> 
                {/* Sidebar */}
                ...
                <main ref={mainScrollRef} className="mt-4 sm:mt-6 flex-1 overflow-y-auto no-scrollbar">
                    <div className="w-full mx-auto space-y-4">
                        ...
                        >
                            ...
                            <div className="flex gap-2 max-w-fit justify-center items-center">
                                ...
                                <button
                                    ...
                                    className={`${isDark ? 'bg-white text-zinc-800 hover:bg-[#f2f3ff] border border-zinc-700' : 'bg-[#696CFF] text-white hover:bg-[#696CFF]/90'} max-w-fit flex justify-center items-center cursor-pointer px-4 py-1.5 rounded-md text-sm hover:shadow transition-all duration-200`}
                                >
                                    Generate
                                </button>
                            </div>
                            ...
                            <div className={`${isDark ? 'text-gray-100' : 'text-gray-900'} mt-2 space-y-3`}>
                                ...
                                {!isLoadingSlides && currentSlide && (
                                    <div className="flex items-stretch gap-3 sm:gap-4">
                                        {/* Left arrow - vertically centered container */}
                                        <div className="hidden sm:flex self-stretch items-center">
                                            <button
                                                type="button"
                                                onClick={handlePrevSlide}
                                                disabled={currentSlideIndex === 0}
                                                className={`flex items-center justify-center h-10 w-10 rounded-full border cursor-pointer ${isDark ? 'border-zinc-700 text-gray-300 hover:bg-zinc-800/60' : 'border-zinc-300 text-zinc-700 hover:bg-zinc-100'} disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200`}
                                            >
                                                <ChevronLeft size={18} />
                                            </button>
                                        </div>
                                                                    ...
                                                                    {isManualEdit && (
                                                                        <>
                                                                            {(() => {
                                                                                const si = currentSlideIndex;
                                                                                const list = histories[si] || [];
                                                                                const idx = historyIndex[si] ?? (list.length ? list.length - 1 : 0);
                                                                                const canUndo = idx > 0;
                                                                                const canRedo = idx < list.length - 1;
                                                                                return (
                                                                                    <div className="flex items-center gap-1">
                                                                                        <button
                                                                                            ...
                                                                                            onClick={handleUndo}
                                                                                            disabled={!canUndo}
                                                                                            className={`${isDark ? 'text-gray-300 hover:bg-zinc-800/60' : 'text-zinc-700 hover:bg-zinc-100'} cursor-pointer flex items-center justify-center h-10 w-10 sm:h-11 sm:w-11 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-200`}
                                                                                            aria-label="Undo"
                                                                                        >
                                                                                            <RotateCcw size={18} />
                                                                                        </button>
                                                            ...
                                                            {isManualEdit ? (
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        setNarration(v);
                                                                        setDraftNarrations((prev) => ({ ...prev, [currentSlideIndex]: v }));
                                                                        pushHistory(currentSlideIndex, v);
                                                                    }}
                                        ...
                                        {/* Right arrow - vertically centered container */}
                                        <div className="hidden sm:flex self-stretch items-center">
                                            <button
                                                type="button"
                        ...
                        {/* Edit/Save controls below the outer card */}
                        {!showPromptBar && (
                            <div className="mt-4 flex justify-end gap-2">
                                {!isManualEdit ? (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            // seed history for current slide
                                            const si = currentSlideIndex;
                                            setHistories((prev) => ({
                                                ...prev,
                                                [si]: prev[si]?.length ? prev[si] : [narration || ""]
                                            }));
                                            setHistoryIndex((prev) => ({
                                                ...prev,
                                                [si]: typeof prev[si] === 'number' ? prev[si] : 0
                                            }));
                                        }}
                                        className={`${isDark ? 'bg-zinc-900/70 text-gray-200 border-zinc-700 hover:bg-zinc-800' : 'bg-white text-zinc-900 border-zinc-300 hover:bg-zinc-100'} inline-flex items-center gap-1 px-4 py-2 rounded-md text-sm sm:text-base font-medium border cursor-pointer`}
                                    >
                                        Edit
                                    </button>
                                ) : (
                                    <button
                                        type="button"
                                        onClick={() => {
                                            // Revert all edits in this session: clear drafts & histories for all slides
                                            setDraftNarrations({});
                                            setHistories({});
                                            setHistoryIndex({});
                                            // Reset current visible narration to the slide's original content
                                            const original = slides[currentSlideIndex]?.narration || "";
                                            setNarration(original);
                                            setOriginalNarration(original);
                                            setIsManualEdit(false);
                                        }}
                                        className={`${isDark ? 'bg-zinc-900/70 text-gray-200 border-zinc-700 hover:bg-zinc-800' : 'bg-white text-zinc-900 border-zinc-300 hover:bg-zinc-100'} inline-flex items-center gap-1 px-4 py-2 rounded-md text-sm sm:text-base font-medium border cursor-pointer`}
                                ...
                                <button
                                    type="button"
                                    onClick={() => {
                                        // Commit draft to slides and clear draft for this slide
                                        setSlides((prev) => {
                                            if (!prev || !prev[currentSlideIndex]) return prev;
                                            const updated = [...prev];
                                    ...
                                    className={`${isDark ? 'bg-zinc-900/70 text-gray-200 border-zinc-700 hover:bg-zinc-800' : 'bg-white text-zinc-900 border-zinc-300 hover:bg-zinc-100'} inline-flex items-center gap-1 px-4 py-2 rounded-md text-sm sm:text-base font-medium border cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed`}
                                >
                                    Save
                                </button>
                            </div>
                        ...
                        {/* Action buttons removed: Generate moved to the toolbar */}
                ...
                {/* Removed fixed bottom-right Edit/Save controls; now inside the card */}
                </div >
            </div>
        </div >
    );
